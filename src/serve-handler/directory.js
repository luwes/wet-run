...
export default (props) => html`<!DOCTYPE html>
  <html lang="en">
    <head>
      ...
      <title>Files within ${props.directory}</title>
    ...
    <body>
      <main>
        <header>
          <h1>
            <i>Index of&nbsp;</i>
            ${props.paths.map((path) => html`<a href="/${path.url}">${path.name}</a>`)}
          </h1>
        </header>
        <ul id="files">
          ${props.files.map((file) => html`
          <li>
            <a href="${file.relative}" title="${file.title}" class="${file.type} ${file.ext}">${file.base}</a>
          </li>`)}
        </ul>
      </main>
    </body>
