...
export async function serveHandler(request, response, config = {}, methods = {}) {
  ...
  if (!stats && (cleanUrl || rewrittenPath)) {
    try {
      let possiblePaths = rewrittenPath ? [rewrittenPath] : getPossiblePaths(relativePath, '.html');
      possiblePaths = possiblePaths.map(item => path.join(current, item));

      if (config.defaults?.[relativePath]) {
        possiblePaths.push(config.defaults[relativePath]);
      }

      const related = await findRelated(possiblePaths, handlers.lstat);
...
const applyRewrites = (requestPath, rewrites = [], repetitive) => {
  ...
  for (let index = 0; index < rewritesCopy.length; index++) {
    ...
    if (target) {
      ...
      return applyRewrites(slashGlob(target), rewritesCopy, true);
    }
  }
...
const sourceMatches = (source, requestPath, allowSegments) => {
  const keys = [];
  const slashed = slashGlob(source);
...
const toTarget = (source, destination, previousPath) => {
  ...
  const normalizedDest = protocol ? destination : slashGlob(destination);
...
const findRelated = async (possiblePaths, originalStat) => {
  let stats = null;
...
const getHeaders = async (handlers, config, current, absolutePath, stats) => {
  ...
  if (customHeaders.length > 0) {
    ...
    for (let index = 0; index < customHeaders.length; index++) {
      ...
      if (sourceMatches(source, slashGlob(relativePath))) {
  ...
  for (const key in headers) {
    if (Object.hasOwn(headers, key) && headers[key] === null) {
      delete headers[key];
    }
  }
